...
export default function BlockEditor({block,blocks,goals,areas,data,close,submit,remove,createGoal}) {
  ...
  return <Modal title={life?'Add Life Commitment':category==='combined'?'Schedule Study + Stretch':stretch?'Schedule Stretch':'Schedule Study'} onClose={close}>
    <form onSubmit={e=>{e.preventDefault();const next={...draft(),...(study?{intention:b.intention || 'peel'}:{}),...(stretch?{environment:b.environment || 'internal',stretchLevel:b.stretchLevel || 'practice',knowledgeIds:b.knowledgeIds || []}:{}),...(category==='combined'?{studyMinutes:studyTime}:{}),...(!life?{repeat:'none'}:{})};
      ...
      const message=validateBlock(next,blocks);if(message){setError(message);return;}submit(next);
    }}>
      ...
      {life && <Field label="Repeat"><select aria-label="Repeat" value={b.repeat || 'none'} onChange={e=>change('repeat',e.target.value)}><option value="none">Does not repeat</option><option value="daily">Every day</option><option value="weekdays">Weekdays</option><option value="weekly">Weekly on this day</option></select></Field>}
      {life && b.repeat && b.repeat!=='none' && <p className="muted">Repeats from this day onward. Editing the repeat updates future occurrences; removing a block skips only that date.</p>}
